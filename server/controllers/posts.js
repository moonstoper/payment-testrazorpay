...
export const register = async (req, res) => { //register the user (not build)
  try {
    console.log("profile");
...
export const payment = async (req, res) => { // creating order instance
  try {
    ...
    const options = { // cretaing options obj to create order instance 
  ...
  } catch (error) { //handling error occurend during any above proccess
    console.log(error);
    res.status(404).send(error);
  }
};
export const paysuccess = async (req, res) => { //after payment verify if payment was successfull or not
  try {
    const {
      orderCreationId,
      razorpayPaymentId,
    ...
    const digest = crypto //crypting and merging order id and paymnet id
      ...
      .digest("hex");
    if (digest !== razorpaySignature) // verifying paymnet 
      return res.status(400).json({ msg: "Transaction not confirmed" });

    res.status(200).json({ // sending if successfull
      msg: "success",
...
export const transactionupdate = async (req, res) => {// updating databse after successfully payment 
  try {
    ...
    if(req.body.payment.objectid !== undefined) //checking if it was pending payment
              ...
              $each: [
                {
                  ...
                  total:(req.body.payment.amount/100)+req.body.payment.duepayment,
